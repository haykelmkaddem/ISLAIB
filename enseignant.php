<?php
session_start();
if(isset($_SESSION["email_en"])){ header("location: welcome_en.php"); exit; }
$errors = ""; 
$has_error = ""; 
$id=isset($_GET['id_en'])?$_GET['id_en']:"erreur";
if($_SERVER["REQUEST_METHOD"] == "POST"):
    require_once "connect.php";
    $email = isset($_POST['email_en'])?htmlspecialchars($_POST['email_en']):'';
    $password = isset($_POST['password_en'])?htmlspecialchars($_POST['password_en']):'';
...
if(empty($errors)) if(!$login->isEnsExist($email))$errors .= "Aucun Compte Trouver<br>";
if(empty($errors)) {
    
    if(!$login->isLoginEnsCorrect($email,$password))$errors .= "l'E-mail ou Mot de passe ne sont pas correct";
    else{
        $_SESSION['id_en'] = $login->getEnsId($email);
        $_SESSION['email_en'] = $email;
        $_SESSION['active'] = 1;
        if(isset($_POST['id_action'])) header("location: welcome_en.php".$_POST['id_action']);        
    ...
    <section class="hero_section ">
      <div class="hero-container container">
        ...
        <div class="hero_img-container">
          <div>
            <img src="images/enseignant1.png" alt="" class="img-fluid">
          </div>
        </div>
      </div>
    </section>
...
<div id="modal-wrapper" class="modal">
  ...
  <form class="modal-content animate" action="enseignant.php" method="POST">
    ...
    <div class="container">
      <input type="text" placeholder="E-mail" name="email_en">
      <input type="hidden" name="id_action" value="">
      <input type="password" placeholder="Enter Password" name="password_en"> 
