    ...
    <ul class="navbar-nav">
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-default" aria-labelledby="navbarDropdownMenuLink-333">
          <a class="dropdown-item" href="ajouter_note.php">Attestaion De Stage</a>
          <a class="dropdown-item" href="modifier_note.php">Resultat De Stage</a>
        </div>
      </li>
    </ul>
<?php
...
$ett="SELECT * FROM etudiant where cin=$cin";
$listett=mysqli_query($conn,$ett);
$ettt = mysqli_fetch_array($listett);
$test=$ettt['id_classe'];


$cll="SELECT * FROM classe where id_classe='$test'";
$listcll=mysqli_query($conn,$cll);
$clll = mysqli_fetch_array($listcll);
$test1=$clll['id_sp'];
$test2=$clll['id_niv'];


$module="SELECT * FROM module  where (id_sp='$test1' and id_niv='$test2' and (id_sem=1 or id_sem=2 ))";
$listmodule=mysqli_query($conn,$module);

$rele = "SELECT * FROM relever where id_classe='$test'";
...
?>
<center>
<table style="width: 90%; text-align: center; margin-top: 30px;" border="2">
  <tr>
    <td rowspan="2">Module</td>
    <td rowspan="2">Type</td>
    <td rowspan="2">Matiére</td>
...
<?php while ($mod = mysqli_fetch_array($listmodule)):
  $modd=$mod['id_mod'];
$matt="SELECT * FROM matiere  where id_mod=$modd";
$listmatt=mysqli_query($conn,$matt);

$mattt = mysqli_fetch_array($listmatt);

$etud=$ettt['cin'];
$m=$mattt['id_mat'];
$note="SELECT * FROM note where (id_et=$etud and id_mat=$m)";
$note1=mysqli_query($conn,$note);
$note2 = mysqli_fetch_array($note1);


  $modat="SELECT COUNT(*) FROM matiere  where id_mod=$modd";
  $modat1=mysqli_query($conn,$modat);
  $modat2= mysqli_fetch_array($modat1);
  $modat3=$modat2[0];
?>
    <tr>
      <td rowspan="<?php echo $modat3 ?>"><?php echo $mod[3] ?></td>
      <td rowspan="<?php echo $modat3 ?>"><?php echo $mod[2] ?></td>
      <td><?php echo $mattt[2]; ?></td>
      <td><?php echo $note2[1]; ?><?php $x=$x+$note2[1]*0.2; ?></td>
      <td><?php echo $note2[2]; ?><?php $y=$y+$note2[2]*0.3; ?></td>
      <td><?php echo $note2[3]; ?><?php $z=$z+$note2[3]*0.5; ?></td>
      <td><?php echo $mattt[1]; ?></td>
      <?php $moymat=$moymat+(($x+$y+$z)*$mattt[1]);
        $coef=$coef+$mattt[1];
       ?>
      <td rowspan="<?php echo $modat3 ?>"><?php echo $mod[1] ?></td>
    </tr>
<?php
  while ($mattt = mysqli_fetch_array($listmatt)) {
    $etud=$ettt['cin'];
$m=$mattt['id_mat'];
$note="SELECT * FROM note where (id_et=$etud and id_mat=$m)";
$note1=mysqli_query($conn,$note);
...
?>
    <tr>
      <td><?php echo $mattt[2]; ?></td>
      <td><?php echo $note2[1]; ?><?php $x=$note2[1]*0.2; ?></td>
      <td><?php echo $note2[2]; ?><?php $y=$note2[2]*0.3; ?></td>
      <td><?php echo $note2[3]; ?><?php $z=$note2[3]*0.5; ?></td>
      <td><?php echo $mattt[1]; ?></td>
      <?php $moymat=$moymat+(($x+$y+$z)*$mattt[1]);
        $coef=$coef+$mattt[1];
       ?>
...
if ($test2==3 or $test2==5) {

$modul1="SELECT * FROM classe  where id_niv=$test2";
$modul2=mysqli_query($conn,$modul1);

while ($module3 = mysqli_fetch_array($modul2)) {

$stnote="SELECT * FROM notestage  where id_et=$cin";
$stnote1=mysqli_query($conn,$stnote);
$stnote2 = mysqli_fetch_array($stnote1);

$stnote3="SELECT * FROM moyenne  where id_et=$cin";
$stnote4=mysqli_query($conn,$stnote3);
$stnote5 = mysqli_fetch_array($stnote4);
 ?>
<tr>
  <td colspan="8" style="right: 0;">
    <table style="width: 100%; text-align: center;">
      <tr>
        <td style="width: 70%;"></td>
        <td style="width: 25%;"><h6>projet fin d'étude :</h3></td>
        <td><?php echo $stnote2[1]; ?> <?php $coef=$coef+30;
        $moymat=$moymat+($stnote2[1]*30); ?></td>
      </tr>

      <tr>
        <td style="width: 70%;"></td>
        <td style="width: 25%;"><h6>Moyenne Générale :</h3></td>
        <td><?php $mo=$moymat/$coef;
        echo round($mo, 2); ?></td>
      </tr>
...
<center>
  <h2>Pas De Résultat Maintenant</h2>
</center>
</div>
