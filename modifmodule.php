    ...
    <ul class="navbar-nav">
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-default" aria-labelledby="navbarDropdownMenuLink-333">
          ...
          <a class="dropdown-item" href="adconsulter_note.php">Calculer</a>
        </div>
      </li>
    </ul>
    <ul class="navbar-nav">
...
$servername="localhost";
  ...
  $moduleid=$_GET['id_mod'];

  $conn=mysqli_connect($servername,$username,$password,$dbname);
$cat="SELECT * FROM specialite";
$listcat=mysqli_query($conn,$cat);

$sem="SELECT * FROM semestre where id_sem<>3";
$listsem=mysqli_query($conn,$sem);

$niv="SELECT * FROM niveau";
$listniv=mysqli_query($conn,$niv);

$mod="SELECT * FROM module where id_mod=$moduleid";
$listmod=mysqli_query($conn,$mod);
...
<center>
    <div class="wrapper" >
         <div class="container">
             ...
             <form action="modmodule.php" method="get">
               <fieldset>
                 
                 <div class="form-group" style="color: black;">
                   <label for="selection">Choisir La Spécialité <span style="color: red;">*</span></label>
                   <select id="custom-select" class="custom-select" name="specialite">
                    <?php while ($row3 = mysqli_fetch_array($listcat)):?>
                       <option value="<?php echo $row3[0]; ?>" style="color: black;"><?php echo $row3[1]; ?></option>
                       <?php endwhile; ?>
                   </select>
                 </div>
                 <div class="form-group" style="color: black;">
                   <label for="selection">Choisir Le Niveau <span style="color: red;">*</span></label>
                   <select id="selection" class="custom-select" name="niveau">
                       <?php while ($row4 = mysqli_fetch_array($listniv)):?>
                       <option value="<?php echo $row4[0]; ?>" style="color: black;"><?php echo $row4[1]; ?></option>
                       <?php endwhile; ?>
                   </select>
                 </div>
                 <div class="form-group" style="color: black;">
                   <label for="selection">Choisir La Semestre <span style="color: red;">*</span></label>
                   <select id="selection" class="custom-select" name="semestre">
                       <?php while ($row5 = mysqli_fetch_array($listsem)):?>
                       <option value="<?php echo $row5[0]; ?>" style="color: black;"><?php echo $row5[1]; ?></option>
                       <?php endwhile; ?>
                   </select>
                 </div>
                 <div class="form-group" style="color: black;">
                   <label for="selection">Choisir Le Type <span style="color: red;">*</span></label>
                   <select id="selection" class="custom-select" name="type">
                       <option value="Fondamentale" style="color: black;">Fondamentale</option>
                       <option value="Transversale" style="color: black;">Transversale</option>
                       <option value="Optionnelle" style="color: black;">Optionnelle</option>
                   </select>
                 </div>

                 <div class="form-group" style="color: black;">
                   <label for="email">Entrez Le Coefficient Du Module <span style="color: red;">*</span></label>
                   <?php $row10 = mysqli_fetch_array($listmod)?>
                   <input type="text" class="form-control" name="coefficient_mod" value="<?php echo $row10[1]; ?>" >
                 </div>


                 <div class="form-group" style="color: black;">
                   <label for="email">Entrez Le Nom Du Module <span style="color: red;">*</span></label>
                   <input type="text" class="form-control" name="nom_mod" value="<?php echo $row10[3]; ?>">
                   <input type="text" class="form-control" name="id_mod" value="<?php echo $row10[0]; ?>" hidden>
                 </div>
               </fieldset>

               <input style="margin-top: 7px; border-radius: 5px; position: absolute;
  right: 30px; background-color: #8EB852; border-color: #8EB852;" type="submit" value="Modifier" class="btn btn-success">
             </form>
         </div>
</div>
