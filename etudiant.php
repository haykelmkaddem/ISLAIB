<?php
session_start();
if(isset($_SESSION["cin"])){ header("location: welcome_et.php"); exit; }
$errors = ""; 
$has_error = ""; 
$id=isset($_GET['cin'])?$_GET['cin']:"erreur";
if($_SERVER["REQUEST_METHOD"] == "POST"):
    require_once "connect.php";
    $email = isset($_POST['cin'])?htmlspecialchars($_POST['cin']):'';
    $password = isset($_POST['pass_et'])?htmlspecialchars($_POST['pass_et']):'';
    
    $login = new MySQL();
    $errors = $login->check_empty(['cin'=>$email,'pass_et'=>$password]);

if(empty($errors)) if(!$login->is_cin_valid($email))$errors.= "CIN n'est pas valide<br>";
if(empty($errors)) if(!$login->isEtudiantExist($email))$errors .= "Aucun Compte Trouver<br>";
if(empty($errors)) {
    
    if(!$login->isLoginEtudiantCorrect($email,$password))$errors .= "CIN ou Mot de passe ne sont pas correct";
    else{
        $_SESSION['cin'] = $login->getEtudiantId($email);
        $_SESSION['cin'] = $email;
        $_SESSION['active'] = 1;
        if(isset($_POST['id_action'])) header("location: welcome_et.php".$_POST['id_action']);        
    ...
    <section class="hero_section ">
      <div class="hero-container container">
        ...
        <div class="hero_img-container">
          <div>
            <img src="images/students.png" alt="" class="img-fluid">
          </div>
        </div>
      </div>
    </section>
...
<div id="modal-wrapper" class="modal">
  ...
  <form class="modal-content animate" action="etudiant.php" method="POST">
    ...
    <div class="container">
      ...
      <input type="password" placeholder="Enter Password" name="pass_et"> 
